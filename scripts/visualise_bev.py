...
def get_window(sample: SampleId, window: int, cfg: PipelineConfig) -> tuple[np.ndarray, str]:
    ...
    Loudly errors if the sample's directory doesn't exist on disk, rather than
    silently producing an empty BEV. (This catches the C1 vs C2 class-prefix
    mismatch: C1 uses M01-M21 but C2 uses A01-A10 and C3 uses E01-E06.)
    """
    sample_dir = cfg.cache_dir.parent.parent / "dataset" / "Radar" / sample.campaign / sample.subject / sample.cls / sample.repetition
    # Cache dir lives at code/cache/, so cache_dir.parent.parent is the project root.
    # But check the *real* dataset path via SampleId.sensor_path which uses RADAR_ROOT directly.
    from rfbc.config import RADAR_ROOT
    sample_dir = RADAR_ROOT / sample.campaign / sample.subject / sample.cls / sample.repetition
    if not sample_dir.is_dir():
        # Try to give a helpful hint about the right class prefix per campaign.
        hint = {
            "C1": "C1 uses M01-M21 (gestures)",
            "C2": "C2 uses A01-A10 (activities), not M-prefixed",
            "C3": "C3 uses E01-E06 (sentiment), not M-prefixed",
        }.get(sample.campaign, "")
...
def _add_recording_area(ax) -> None:
    """Overlay the 2 m x 6 m recording rectangle (dashed).

    The recording area has its long axis along WORLD Y, not X.
    Ground sensors form a ring with 3 sensors on each long side (south at
    y~-3, north at y~+3) and one sensor at each short end (east at x~+2.6,
    west at x~-2.9). So the corridor where the person walks/stands is
    x in [-1, +1] (2 m wide) and y in [-3, +3] (6 m long).
    """
    rect = Rectangle(
        (-1.0, -3.0), 2.0, 6.0,
...
def main() -> None:
    ...
    p.add_argument("--window", type=int, default=0,
                   help="Which window of the sample (0-indexed)")
    p.add_argument("--feature", type=int, default=0,
                   help="Which per-cell feature: 0=count, 1=mean density, 2=max density")
    p.add_argument("--montage-frames", type=int, default=9,
                   help="Number of frames in the temporal montage")
    p.add_argument("--out", default=None,
                   help="Output PNG path (default auto-named into code/outputs)")
    args = p.parse_args()
    ...
    bev, source = get_window(sample, args.window, cfg)
    print(f"loaded BEV from: {source}", flush=True)
    print(f"  shape: {bev.shape}  dtype: {bev.dtype}", flush=True)
    print(f"  non-zero cells: {(bev > 0).sum()} of {bev.size} "
          f"({100 * (bev > 0).mean():.3f}%)", flush=True)

    bev_5d = unflatten_channels(bev, cfg)
    fig = make_figure(bev_5d, sample, args.window, args.feature, cfg,
    ...
    if args.out is None:
        ...
        out_path = outputs_dir / (
            f"bev_{sample.campaign}_{sample.subject}_{sample.cls}_{sample.repetition}"
            f"_w{args.window}_feat{args.feature}.png"
        )
    else:
